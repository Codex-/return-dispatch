...
async function dispatchWorkflow(distinctId) {
  try {
    const response = await octokit.rest.actions.createWorkflowDispatch({
      ...
      ref: config.ref,
      inputs: __spreadProps(__spreadValues({}, config.workflowInputs ? config.workflowInputs : void 0), {
    ...
    core3.info(`Successfully dispatched workflow:
...
` : ``) + `  Distinct ID: ${distinctId}`);
    ...
    if (error4 instanceof Error) {
      core3.error(`dispatchWorkflow: An unexpected error has occurred: ${error4.message}`);
...
async function getWorkflowId(workflowFilename) {
  ...
  try {
    ...
    const workflowId = (_a = response.data.workflows.find((workflow) => new RegExp(workflowFilename).test(workflow.path))) == null ? void 0 : _a.id;
    ...
    if (error4 instanceof Error) {
      core3.error(`getWorkflowId: An unexpected error has occurred: ${error4.message}`);
      error4.stack && core3.debug(error4.stack);
    }
    throw error4;
  }
...
async function getWorkflowRunIds(workflowId) {
  ...
  Runs Fetched: [${runIds}]`);
    ...
    if (error4 instanceof Error) {
      core3.error(`getWorkflowRunIds: An unexpected error has occurred: ${error4.message}`);
      error4.stack && core3.debug(error4.stack);
    }
    throw error4;
  }
...
async function getWorkflowRunJobSteps(runId) {
  try {
    ...
    core3.debug(`Fetched Workflow Run Job Steps:
  Repository: ${config.owner}/${config.repo}
  ...
  Jobs Fetched: [${jobs.map((job) => job.id)}]  Steps Fetched: [${steps}]`);
    ...
    if (error4 instanceof Error) {
      core3.error(`getWorkflowRunJobs: An unexpected error has occurred: ${error4.message}`);
      error4.stack && core3.debug(error4.stack);
    }
    throw error4;
  }
...
async function run() {
  try {
    ...
    while (elapsedTime < timeoutMs) {
      ...
      const workflowRunIds = await retryOrDie(() => getWorkflowRunIds(workflowId), WORKFLOW_FETCH_TIMEOUT_MS > timeoutMs ? timeoutMs : WORKFLOW_FETCH_TIMEOUT_MS);
      core4.debug(`Attempting to get step names for Run IDs: [${workflowRunIds}]`);
  ...
  URL: ${url}`);
      ...
      core4.info(`Exhausted searching IDs in known runs, attempt ${attemptNo}...`);
